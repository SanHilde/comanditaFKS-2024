...
export class QrService {
  ...
  generarTexto(tipo: tipoQr, objeto: any) {
    switch (tipo) {
      ...
      case tipoQr.MesaUno:
        return tipoQr.MesaUno + objeto.numero + objeto.tipo + objeto.cantidadComensales;
      case tipoQr.MesaDos:
    ...
    return new Promise((resolve, reject) => {
      const procesarQr = (qr: string) => {
        if (qr.includes("@")) {
          ...
          const tipo = qr.substring(0, qr.indexOf(" ") > -1 ? qr.indexOf(" ") : qr.length).toUpperCase();
  ...
  buscarAccion(tipo: string, id: string) {
    switch (tipo) {
      case tipoQr.Producto:
        // Acciones específicas para Producto
        break;
      case tipoQr.Ingreso:
